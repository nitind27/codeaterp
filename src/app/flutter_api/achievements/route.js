...
export async function GET(req) {
  try {
    ...
    console.error('Flutter Get achievements error:', error);
...
export async function POST(req) {
  try {
    ...
    if (!title || !awardedOn) {
      return errorResponse('Title and award date are required', 400);
    }
    ...
    if (employeeId) {
      ...
      if (employee.length === 0) {
        return errorResponse('Invalid employee selected', 400);
      }
    ...
    await logActivity(
      ...
      'achievement',
      result.insertId,
      `Created achievement via Flutter: ${title}`,
      req
    );
    ...
    console.error('Flutter Create achievement error:', error);
    return errorResponse('Internal server error', 500);
  }
}
