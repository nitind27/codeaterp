...
export async function GET(req, { params }) {
  try {
    ...
    console.error('Flutter Get employee error:', error);
...
export async function PUT(req, { params }) {
  try {
    ...
    await logActivity(user.id, 'update_employee', 'employee', id, 
      `Updated employee profile via Flutter`, req);

    return successResponse({
      message: 'Employee updated successfully'
    });
  } catch (error) {
    console.error('Flutter Update employee error:', error);
    return errorResponse('Internal server error', 500);
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    await logActivity(authResult.user.id, 'delete_employee', 'employee', id, 
      `Deleted employee via Flutter: ${employees[0].employee_id}`, req);

    return successResponse({
      message: 'Employee deleted successfully'
    });
  } catch (error) {
    console.error('Flutter Delete employee error:', error);
    return errorResponse('Internal server error', 500);
  }
}
