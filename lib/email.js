...
export async function sendFeesReminderEmail(data) {
  ...
  const subject = `⚠️ Fees Payment Reminder — Please Pay ${fmtAmt(requestAmount)} by ${dueDateFmt || 'Soon'} | Codeat Infotech`;
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.10);overflow:hidden;max-width:600px;">
  ...
  <tr>
    <td style="background:linear-gradient(135deg,#1A656D 0%,#0A2A2D 100%);padding:36px 32px 28px;text-align:center;">
      ...
      <p style="color:#8db2b6;margin:8px 0 0;font-size:13px;">Codeat Infotech · Internship Management</p>
    </td>
  </tr>
  ...
  <tr><td style="padding:32px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#fff3e0,#ffe0b2);border:2px solid #ff9800;border-radius:14px;margin:0 0 24px;overflow:hidden;">
      <tr>
        <td style="background:#e65100;padding:10px 20px;">
          <span style="color:#fff;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">⚠️ Payment Requested</span>
        </td>
      </tr>
      <tr>
        <td style="padding:20px;text-align:center;">
          ...
          ${dueDateFmt ? `<p style="color:#e65100;font-size:14px;font-weight:700;margin:10px 0 0;">📅 By: <strong>${dueDateFmt}</strong></p>` : ''}
          ${willClear ? `<p style="color:#2e7d32;font-size:13px;font-weight:600;margin:10px 0 0;background:#e8f5e9;padding:6px 14px;border-radius:20px;display:inline-block;">✅ This will clear your fees completely!</p>` : ''}
        </td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e0e0e0;border-radius:12px;overflow:hidden;margin:0 0 24px;">
      <tr>
        <td style="background:#1A656D;padding:10px 16px;">
          <span style="color:#fff;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">📊 Fees Breakdown</span>
        </td>
      </tr>
      <tr>
        <td style="padding:0;">
    ...
    <div style="background:#e8f4fd;border-left:4px solid #1976d2;padding:14px 16px;border-radius:0 8px 8px 0;margin:0 0 20px;">
      <p style="color:#1565c0;font-size:12px;font-weight:700;margin:0 0 5px;text-transform:uppercase;">📝 Note from Admin</p>
  ...
  <tr>
    <td style="background:#f8f9fa;padding:18px 32px;text-align:center;border-top:1px solid #e9ecef;">
      <p style="color:#aaa;font-size:11px;margin:0;">© ${new Date().getFullYear()} Codeat Infotech. All rights reserved.</p>
  ...
  const paidPct   = totalFees > 0 ? Math.round((paidAmount / totalFees) * 100) : 0;
  const barWidth  = Math.max(2, paidPct);
  const isPaid    = remainingAmount <= 0;
  const fmtAmt    = (n) => '₹' + parseFloat(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const today     = new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' });

  const subject = isPaid
    ? `✅ Fees Fully Paid — Thank You! | Codeat Infotech`
    : `⚠️ Fees Payment Reminder — ₹${parseFloat(remainingAmount).toLocaleString('en-IN')} Pending | Codeat Infotech`;
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;padding:40px 0;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.10);overflow:hidden;max-width:600px;">

      <!-- HEADER -->
      <tr>
        <td style="background:linear-gradient(135deg,#1A656D 0%,#0A2A2D 100%);padding:36px 32px 28px;text-align:center;">
          <div style="display:inline-block;background:rgba(255,255,255,0.12);border-radius:50%;width:60px;height:60px;line-height:60px;font-size:28px;margin-bottom:12px;">💰</div>
          <h1 style="color:#fff;margin:0;font-size:24px;font-weight:700;letter-spacing:-0.3px;">
            ${isPaid ? 'Fees Fully Paid!' : 'Fees Payment Reminder'}
          </h1>
          <p style="color:#8db2b6;margin:8px 0 0;font-size:14px;">Codeat Infotech · Internship Management</p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:32px;">
          ...
          ${isPaid
            ? `<p style="color:#2e7d32;font-size:15px;line-height:1.7;margin:0 0 20px;background:#f1f8e9;border-left:4px solid #66bb6a;padding:14px 16px;border-radius:0 8px 8px 0;">
                🎉 Congratulations! Your internship fees have been <strong>fully paid</strong>. Thank you for your timely payments!
               </p>`
            : `<p style="color:#555;font-size:15px;line-height:1.7;margin:0 0 20px;">
                This is a friendly reminder regarding your pending internship fees. Please find the details below and arrange the payment at your earliest convenience.
               </p>`
          }

          <!-- FEES SUMMARY CARD -->
          <table width="100%" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#f8fffe,#e8f5f5);border:2px solid #1A656D;border-radius:12px;overflow:hidden;margin:0 0 24px;">
            <tr>
              <td style="background:#1A656D;padding:12px 20px;">
                <span style="color:#fff;font-size:13px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;">📊 Fees Summary</span>
              </td>
            </tr>
            <tr>
              <td style="padding:20px;">
                <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
                  <tr>
                    <td style="color:#666;padding:10px 0;border-bottom:1px solid #dde8e8;width:50%;">Total Course Fees</td>
                    <td style="color:#1A656D;font-weight:700;text-align:right;padding:10px 0;border-bottom:1px solid #dde8e8;">${fmtAmt(totalFees)}</td>
                  </tr>
                  <tr>
                    <td style="color:#666;padding:10px 0;border-bottom:1px solid #dde8e8;">Amount Paid</td>
                    <td style="color:#2e7d32;font-weight:700;text-align:right;padding:10px 0;border-bottom:1px solid #dde8e8;">${fmtAmt(paidAmount)}</td>
                  </tr>
                  <tr>
                    <td style="color:#666;padding:10px 0;border-bottom:1px solid #dde8e8;">Amount Remaining</td>
                    <td style="color:${isPaid ? '#2e7d32' : '#c62828'};font-weight:700;text-align:right;padding:10px 0;border-bottom:1px solid #dde8e8;">${fmtAmt(remainingAmount)}</td>
                  </tr>
                  <tr>
                    <td style="color:#666;padding:10px 0;">Payment Progress</td>
                    <td style="text-align:right;padding:10px 0;">
                      <span style="color:#1A656D;font-weight:700;">${paidPct}% Paid</span>
                    </td>
                  </tr>
                </table>

                <!-- Progress Bar -->
                <div style="background:#e0e0e0;border-radius:99px;height:10px;margin-top:8px;overflow:hidden;">
                  <div style="background:${isPaid ? '#43a047' : '#1A656D'};width:${barWidth}%;height:10px;border-radius:99px;"></div>
                </div>
              </td>
            </tr>
          </table>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff3e0;border:1px solid #ffb74d;border-radius:10px;margin:0 0 24px;">
            <tr>
              <td style="padding:18px 20px;text-align:center;">
                <p style="color:#e65100;font-size:13px;font-weight:600;margin:0 0 6px;text-transform:uppercase;letter-spacing:0.5px;">⚠️ Pending Amount</p>
                <p style="color:#bf360c;font-size:28px;font-weight:800;margin:0;">${fmtAmt(remainingAmount)}</p>
                ${dueDate ? `<p style="color:#e65100;font-size:13px;margin:8px 0 0;">Please pay by <strong>${new Date(dueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</strong></p>` : ''}
              </td>
            </tr>
          </table>
          ` : ''}
          ...
          <!-- INTERN DETAILS -->
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f9fa;border-radius:10px;margin:0 0 24px;">
            <tr>
              <td style="padding:16px 20px;">
                <p style="color:#888;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin:0 0 10px;">Intern Details</p>
                <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;">
                  <tr>
                    <td style="color:#666;padding:4px 0;width:40%;">Name</td>
                    <td style="color:#333;font-weight:600;padding:4px 0;">${internName}</td>
                  </tr>
                  <tr>
                    <td style="color:#666;padding:4px 0;">Employee ID</td>
                    <td style="color:#333;font-weight:600;padding:4px 0;">${employeeId}</td>
                  </tr>
                  <tr>
                    <td style="color:#666;padding:4px 0;">Reminder Date</td>
                    <td style="color:#333;font-weight:600;padding:4px 0;">${today}</td>
                  </tr>
                </table>
              </td>
            </tr>
          ...
          ${notes ? `
          <!-- ADMIN NOTE -->
          <div style="background:#e8f4fd;border-left:4px solid #1976d2;padding:14px 16px;border-radius:0 8px 8px 0;margin:0 0 24px;">
            <p style="color:#1565c0;font-size:13px;font-weight:700;margin:0 0 6px;">📝 Note from Admin</p>
            <p style="color:#333;font-size:14px;margin:0;line-height:1.6;">${notes}</p>
          </div>
          ` : ''}
          ...
          <div style="text-align:center;margin:28px 0;">
            <p style="color:#555;font-size:14px;margin:0 0 16px;">Please log in to the ERP portal to view your complete payment history and download receipts.</p>
            <a href="${process.env.NEXT_PUBLIC_API_URL || 'https://erp.codeatinfotech.com'}/fees"
               style="display:inline-block;background:linear-gradient(135deg,#1A656D,#175b62);color:#fff;text-decoration:none;padding:14px 36px;border-radius:8px;font-size:15px;font-weight:700;box-shadow:0 4px 14px rgba(26,101,109,0.35);">
              View My Fees →
            </a>
          ...
          <p style="color:#333;font-size:14px;margin:20px 0 0;">
            Regards,<br/>
            <strong style="color:#1A656D;">${adminName || 'Codeat Infotech Finance Team'}</strong><br/>
            <span style="color:#888;font-size:12px;">Codeat Infotech</span>
          </p>
      ...
      <tr>
        <td style="background:#f8f9fa;padding:20px 32px;text-align:center;border-top:1px solid #e9ecef;">
          <p style="color:#aaa;font-size:11px;margin:0;">© ${new Date().getFullYear()} Codeat Infotech. All rights reserved.</p>
          <p style="color:#bbb;font-size:11px;margin:6px 0 0;">This is an automated reminder. Please do not reply to this email.</p>
        </td>
      </tr>
